...
Gate 5 synthesis from the 2026-07-11 governance run on the Decision 15 docs commit. Each track 2f implementation PR **must carry the automated reproductions mapped to it below** (P0 = blocks that PR's merge); the letters refer to Decision 15's pre-commitments (a)–(j) in `docs/adr-0022-execution-plan.md`.
...
- **CH-5 — Cap exhaustion on shared token** (verifies d+h+j). Drive one principal to the per-principal cap, a second to the global cap. Success: cap+1 `initialize` rejected with an A4 error; no live session/stream evicted; streamed POSTs debit the same budget as GET channels. **P0 PR 2.**
