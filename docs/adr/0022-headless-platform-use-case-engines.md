...
Related: ADR-0021 (Spark/Reflex architecture), ADR-0017 (management-group confinement of list reads), ADR-0006/0008/0012 (Postgres substrate and store contract), `docs/agentic-first-principle.md` (A1–A4), `docs/auth-architecture.md`, `docs/mcp-server.md`.

## Binding status

Nothing in this ADR binds reviews or blocks PRs until its status is **accepted**. On acceptance, Decisions 1–4 bind **prospectively** — they govern new and changed capabilities from the acceptance date and do not retroactively condemn the grandfathered surfaces inventoried below. Decisions 5–7 describe direction whose binding force arrives with their named follow-ups.
...
- Engine principals authenticate with their own credentials and carry their own RBAC grants, assigned least-privilege — an engine holding the union of its users' permissions is a design defect.
- **Delegation is server-verifiable.** When an engine principal acts on a specific operator's initiative, the delegated identity must be established by a server-issued artifact (token exchange or equivalent) — never by an engine-asserted header or field. Any action serving a specific operator's request MUST be performed as delegated; autonomous mode is for engine-scheduled work only. A confused-deputy engine must not be able to launder an operator-initiated action through its own broader grants.
...
2. **`vuln_scan` plugin + server-side NVD sync** (capability-map 9.4, shipped). In-server interpretation by Decision 2's test; grandfathered until re-homed into the first use-case module. Roadmap phases 18.1 (CVE lifecycle store), 18.2 (auditor-ready compliance bundles), and 18.5 (SBOM CVE linkage) are **boundary-affected**: on acceptance they must be re-evaluated against Decision 2 before implementation.
3. Exceptions recorded after acceptance accrete here (the "exception ledger" the standing review question refers to).
