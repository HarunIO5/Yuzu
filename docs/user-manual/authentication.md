...
- **OIDC/SSO account:** ⚠️ **JIT elevation for OIDC sessions is temporarily unavailable** on this release (the #1837/#1857 identity re-key; restoration tracked in #1852). An OIDC session's principal is now the immutable `oidc:<iss>#<sub>`, which has no local `users` row, so the elevate request is denied at the eligibility gate. To elevate today, use a local-authenticated session with local TOTP. *The `amr` behaviour below is the intended path #1852 restores:* if the operator's *current* SSO login was authenticated with IdP MFA (asserted via the OIDC `amr` claim), that assertion satisfies the second-factor requirement — **no local TOTP enrollment needed**. A single-factor SSO login (no IdP MFA) is still denied. This is controlled by `--jit-oidc-amr-elevation` (default enabled). Disabling it (`--no-jit-oidc-amr-elevation`) means **OIDC sessions cannot use JIT elevation at all** — an OIDC session cannot present a local TOTP step-up (its step-up challenge is re-authenticating via SSO, not a TOTP code), so the operator must elevate from a local-authenticated session with local TOTP instead.
