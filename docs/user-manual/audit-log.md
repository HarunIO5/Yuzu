...
| `role.elevation.granted` | User | A pre-authorized operator activated a JIT admin elevation via `POST /api/v1/elevate` (SOC 2 CC6.3/CC6.6). `principal`=the operator, `principal_role=admin` (the effective role for the window), `result=ok`, `detail=duration_secs=<N> justification=<sanitised, ≤1KiB> expires_at=<RFC3339 UTC>`. The `expires_at` reflects any clamp to the session's own absolute lifetime (follow-up B). The grant was MFA-step-up-gated. |
