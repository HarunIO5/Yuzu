...
| `rdp_control.set_state` | n/a (RDP control is Windows-only) | n/a | service account in `Administrators` group (elevated token): writes `HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server`, toggles the Remote Desktop firewall rule group via `INetFwPolicy2`, and starts `TermService` (`SERVICE_START`). Mirrors the `registry.set_value` + `quarantine.*` grants — no new right beyond those rows. In the **intended** least-privilege model the agent runs as `NT SERVICE\YuzuAgent` (not in `Administrators`), so this action would require the out-of-band grant below. **Today the service runs as LocalSystem (#1442 — see the Correction above), so this succeeds with no extra grant.** **`scripts/install-agent-user.ps1` does NOT grant this** — its `$RequiredGroups` is only `Event Log Readers` / `Performance Monitor Users` / `Performance Log Users`, and `-Check` verifies only those. The Administrators membership must be granted out of band, e.g. GPO **Restricted Groups** / Intune **Local Users and Groups** policy, or `Add-LocalGroupMember -Group 'Administrators' -Member 'NT SERVICE\YuzuAgent'` (virtual service accounts can be added by SID), and verified with `Get-LocalGroupMember Administrators`. `rdp_control.status` is read-only but `IsRuleGroupEnabled` still requires local admin in practice. A non-elevated agent reports `reg_status\|error:5` (never a silent no-op). **Dispatch gate:** the *only* control enforced on every path (incl. `POST /api/command` and MCP `execute_instruction`) is the `Execution:Execute` RBAC permission — the definition's `role-gated` approval and `executeRoles` apply only on `POST /api/instructions/{id}/execute`. Restrict `Execution:Execute` and MCP token scope, and keep a Guardian `fDenyTSConnections=1` backstop deployed. |
| Guardian `service` guard **enforce** (`service-running`/`service-stopped` write-back via `StartService`/`ControlService`) | n/a (enforce is Windows-only; Linux is **observe-only** — see below) | n/a | `SERVICE_START` / `SERVICE_STOP` on the **target service's DACL** — a per-service right, not a global LSA privilege. The virtual service account holds these on its own service; controlling a third-party service depends on that service's ACL. On denial the guard falls back to detect-only and reports `remediation.failed` (no self-elevation). Enforce-stop of security/critical-infra/self services is refused at authoring (`dangerous_enforce_service_stop` denylist). |
| Guardian `service` guard **observe** (Linux, `SystemdServiceGuard`) | n/a | reading a systemd unit's `ActiveState` over the **system D-Bus** (`sd_bus_open_system` + `Manager.Subscribe` + `LoadUnit` + a `PropertiesChanged` match) needs **no elevation, no sudoers entry, no capability** — unit-state queries are unprivileged for any local user. No grant required for the observe path. | n/a | **Linux enforce (mask/stop) is deferred**; when it lands it will require a scoped polkit grant (`org.freedesktop.systemd1.manage-units`), not blanket root — that row will be added with the enforce PR. |
...
| Service registration | as part of package install (`systemctl enable`, `launchctl bootstrap`, `sc.exe create`) | the dev operator runs `scripts/start-UAT.sh` which doesn't register a service — the agent runs as a foreground process. Pass `--as-user _yuzu` (macOS) or `--as-user yuzu` (Linux) to launch the agent under the unprivileged account via `sudo -u`; the script's pre-flight verifies the user, sudoers grant, binary readability, and parent-path traversability before attempting to start. |
...
- **A boot AutoLogger** named `YuzuProcBoot` — a persistent registry entry under `HKLM\System\CurrentControlSet\Control\WMI\Autologger\YuzuProcBoot` that makes the kernel start a circular, 16 MB, FlushTimer-enabled Kernel-Process `.etl` (`<data_dir>\procboot.etl`) early on every boot, so the agent can backfill the boot window it would otherwise miss. It is configured by the **production InnoSetup installer** (scoped to the `advanced` component that ships `tar.dll`) and by the developer install script (`install-agent-user.ps1`); both run the same `New-AutologgerConfig` + `Add-EtwTraceProvider` recipe and must be kept in sync, and the `.etl` path matches the service's `--data-dir`. A security team can confirm it after a reboot with `logman query YuzuProcBoot -ets` (the running trace session) or by reading the registry key above. Uninstall — both the installer's `[UninstallRun]` and `install-agent-user.ps1 -Uninstall` — removes the boot-start config, **stops the running `YuzuProcBoot` session** (freeing one of the ~64 system ETW session slots), and deletes the `.etl`.

Neither captures command lines (names-only); the boot-window events additionally carry no user. The capture respects the `process_enabled` toggle on **both** paths: an operator who disables process collection gets no boot-backfill insert on the next restart, and the live ETW session — which keeps running — has its buffered events drained-and-discarded each cycle while disabled, so a paused window is never persisted when collection is re-enabled.
