...
`ManagementGroupStore` supports group-scoped role assignments. `assign_role()` / `unassign_role()` bind principals to roles within a specific group. `get_visible_agents()` resolves the candidate agent set for a user's group-scoped roles (the wiring exists; it is not yet the effective list gate — see below). REST endpoints at `/api/v1/management-groups/{id}/roles` (GET/POST/DELETE). RBAC integration enforces group-scoped visibility on **per-device** routes today; confinement of **list/fan-out** reads (agent lists, responses, inventory) is **not yet effective** — see ADR-0017 (the admit-then-filter list gate).
