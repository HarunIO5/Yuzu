...
- **Reserved on-behalf-of headers rejected (ADR-0022):** MCP registers on the same httplib instance as REST, so the pre-routing chokepoint rejects any `/mcp/v1/` request carrying a reserved on-behalf-of header with `403` before the JSON-RPC layer runs — see `docs/auth-architecture.md` ("On-behalf-of assertions rejected") for the reserved-name list.
