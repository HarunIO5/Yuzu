...
| **SCIM v2 provisioning** (auto-provision/deprovision from IdP) | "Periodic access reviews" automation | CC6.2/6.8 | **SHIPPED (Users slice)** — `--scim-enable`/`--scim-token` (fail-closed: refuses to start without a token, or with `--no-https`); every `/scim/v2/*` route (including discovery) bearer-authed constant-time, its own `scim-service` audit principal. `POST /scim/v2/Users` provisions at the fixed `role=user` (SSO login, discarded local password); `PATCH .../{id}` `active:false`/`active:true` deprovisions (soft-delete + session-revoke cascade) / reactivates (lockout cleared, MFA NOT restored). **Provenance guard** (`users.provisioning_source`, auth.db migration v7) makes every deactivate/reactivate/delete re-verify `provisioning_source == "scim"` before mutating, refusing `404` (never `403`) on mismatch — a locally-created admin or the break-glass account can never be touched by an IdP push, and SCIM users are always `role=user` so a compromised IdP can't create/remove an admin. Storage rides `auth.db` (own `"scim"` migration component), not a new store. **Deferred:** Groups→role mapping, `userName` rename, per-route rate-limiting, SCIM-token-at-rest encryption. See `docs/auth-architecture.md` "SCIM v2 provisioning". |
...
7. ~~**SCIM v2 provisioning**~~ **DONE (Users slice)** — auto-create/deactivate/
   ...
   deactivated by SCIM. **Remaining (next slice): Groups→role mapping**
   (every SCIM user is `role=user` regardless of IdP group membership),
   `userName` rename, per-route rate-limiting, SCIM-token-at-rest encryption.
